Olá 👋

Meu nome é Gabriel da Cruz, sou profissional de Tecnologia da Informação com mais de 6 anos de experiência em suporte técnico, infraestrutura de TI, administração de ambientes corporativos e atendimento a usuários. Atualmente, estou em transição para a área de Desenvolvimento de Software, ampliando minha atuação dentro do universo da tecnologia.

Atualmente curso o 4º semestre de Análise e Desenvolvimento de Sistemas, onde venho fortalecendo minha base em programação e adquirindo experiência prática com ferramentas e tecnologias modernas utilizadas no mercado.

Um pouco mais sobre mim:

🌱 Estou sempre em evolução, buscando novos conhecimentos e explorando tecnologias atuais.
👨‍💻 Ao longo da minha formação e projetos, já desenvolvi experiência prática com:
 - JavaScript
 - TypeScript
 - Node.js
 - Next.js
 - React
 - MySQL
 - Python
🧰 Possuo ampla vivência em suporte e infraestrutura, incluindo:
Manutenção e diagnóstico de hardware (notebooks e desktops)
Suporte técnico ao usuário e atendimento interno
Operação e suporte a sistemas ERP
Instalação e manutenção de impressoras
Configuração de roteadores e redes
Experiência com Windows e outros sistemas operacionais
Uso de ferramentas de acesso remoto
Administração de servidores
Hyper-V
Azure
Gestão de usuários, permissões de rede e domínio
Conhecimentos em firewall e segurança de rede
Gestão e monitoramento de backups
Contato com fornecedores e negociação
Controle e inventário de ativos de TI
🚀 Atualmente busco novas oportunidades nas áreas de Suporte Técnico, Infraestrutura ou Desenvolvimento de Software, onde eu possa aplicar todo o meu conhecimento prático em TI, evoluir profissionalmente e ampliar minha atuação no desenvolvimento de soluções tecnológicas.
📫 Entre em contato comigo se quiser trocar ideias, conversar sobre tecnologia ou colaborar em projetos:
LinkedIn: [GabrielCruz](https://www.linkedin.com/in/gabriel-da-cruz-a7082b294/)
